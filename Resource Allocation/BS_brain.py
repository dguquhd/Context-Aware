# implement the DNN in Each D2D and the DQN at the BS for the D-Decision scheme with Real Feedback
...
class BS:
    ...
    def _create_model(self):
        ...
        Num_Compress_NN_Inner_Layer1 = 32
        Num_Compress_NN_Inner_Layer2 = 64
        Num_Compress_NN_Inner_Layer3 = 32

        # implement DNN for 1st D2D
        D1_Input = Input(shape=(Num_D2D_Input,), name='D1_Input')
        D1 = Dense(Num_Compress_NN_Inner_Layer1, activation='relu')(D1_Input)
        D1 = Dense(Num_Compress_NN_Inner_Layer2, activation='relu')(D1)
        D1 = Dense(Num_Compress_NN_Inner_Layer3, activation='relu')(D1)
        ...
        D2 = Dense(Num_Compress_NN_Inner_Layer1, activation='relu')(D2_Input)
        D2 = Dense(Num_Compress_NN_Inner_Layer2, activation='relu')(D2)
        D2 = Dense(Num_Compress_NN_Inner_Layer3, activation='relu')(D2)
        ...
        D3 = Dense(Num_Compress_NN_Inner_Layer1, activation='relu')(D3_Input)
        D3 = Dense(Num_Compress_NN_Inner_Layer2, activation='relu')(D3)
        D3 = Dense(Num_Compress_NN_Inner_Layer3, activation='relu')(D3)
        ...
        D4 = Dense(Num_Compress_NN_Inner_Layer1, activation='relu')(D4_Input)
        D4 = Dense(Num_Compress_NN_Inner_Layer2, activation='relu')(D4)
        D4 = Dense(Num_Compress_NN_Inner_Layer3, activation='relu')(D4)
        ...
        BS_Dense1 = Dense(1000, activation='relu', name='BS_Dense1')(BS_Input)
        BS_Dense2 = Dense(800, activation='relu', name='BS_Dense2')(BS_Dense1)
        BS = Dense(600, activation='relu', name='BS')(BS_Dense2)
        Num_BS_Output = self.num_Output
        BS_Output = Dense(Num_BS_Output, activation='linear', name='BS_Output')(BS)

        # implement Decision DNN for 1st D2D
        Num_D2D_Decide_Output = self.num_CH

        Num_DQN_Inner_Layer1 = 160
        Num_DQN_Inner_Layer2 = 80
        Num_DQN_Inner_Layer3 = 40

        D1_Decide_Input = keras.layers.concatenate([D1_Input, BS_Output], name='D1_Decide_Input')
        D1_Decide = Dense(Num_DQN_Inner_Layer1, activation='relu')(D1_Decide_Input)
        D1_Decide = Dense(Num_DQN_Inner_Layer2, activation='relu')(D1_Decide)
        D1_Decide = Dense(Num_DQN_Inner_Layer3, activation='relu')(D1_Decide)
        ...
        D2_Decide_Input = keras.layers.concatenate([D2_Input, BS_Output], name='D2_Decide_Input')
        D2_Decide = Dense(Num_DQN_Inner_Layer1, activation='relu')(D2_Decide_Input)
        D2_Decide = Dense(Num_DQN_Inner_Layer2, activation='relu')(D2_Decide)
        D2_Decide = Dense(Num_DQN_Inner_Layer3, activation='relu')(D2_Decide)
        ...
        D3_Decide_Input = keras.layers.concatenate([D3_Input, BS_Output], name='D3_Decide_Input')
        D3_Decide = Dense(Num_DQN_Inner_Layer1, activation='relu')(D3_Decide_Input)
        D3_Decide = Dense(Num_DQN_Inner_Layer2, activation='relu')(D3_Decide)
        D3_Decide = Dense(Num_DQN_Inner_Layer3, activation='relu')(D3_Decide)
        ...
        D4_Decide_Input = keras.layers.concatenate([D4_Input, BS_Output], name='D4_Decide_Input')
        D4_Decide = Dense(Num_DQN_Inner_Layer1, activation='relu')(D4_Decide_Input)
        D4_Decide = Dense(Num_DQN_Inner_Layer2, activation='relu')(D4_Decide)
        D4_Decide = Dense(Num_DQN_Inner_Layer3, activation='relu')(D4_Decide)
...
class Agent:
    ...
    def __init__(self, num_d2d, num_ch, num_neighbor, num_d2d_feedback, environment, curr_rl_config):
        ...
        self.num_BS_Output = curr_rl_config.Num_BS_Output
    ...
    def test_run(self, num_episodes, num_test_step, demand_size, opt_flag):
        ...
        demand_marl = self.demand_size * np.ones([self.num_Episodes, self.num_Test_Step + 1, self.num_D2D, self.num_Neighbor])
        ...
        # tracking the simulation
        Run_Episode_Interval = 200
        Run_Step_Interval = 500

        for Episode_loop in range(self.num_Episodes):

            # start a new game for each episode
            self.env.new_random_game(self.num_D2D)
            self.demand = self.demand_size * np.ones((self.num_D2D, self.num_Neighbor))
            ...
            for Run_loop in range(self.num_Test_Step):
                ...
                self.demand -= V2V_Rate * self.time_fast * self.bandwidth * 1e6
                self.demand[self.demand < 0] = 0
                demand_marl[Episode_loop, Run_loop + 1, :, :] = self.demand
                V2V_success = (1 / np.exp(-r0 / np.average(10 * np.log10(2 ** V2V_Rate - 1)))) * (
                            self.demand_size / (np.sum(V2V_Rate) * self.bandwidth * 1 / self.time_slow))
                if Run_loop % 120 == 0:
                    print('Episode:', Episode_loop, 'V2V_rate:\n', Per_V2V_Rate[Episode_loop, Run_loop, :])
        ...
        if RA_Flag and Opt_Flag:
            print('Finish Running Real-FB D-Decision Test with Optimal Scheme!')
            ...
            if RA_Flag:
                print('Finish Running Real-FB D-Decision Test without Optimal Scheme!')
                return V2V_success, demand_marl, Expect_Return, Reward, Per_V2V_Rate, Per_V2I_Rate, \
                       Per_V2B_Interference, \
                       RA_Expect_Return, RA_Reward, RA_Per_V2V_Rate, RA_Per_V2I_Rate, \
                       RA_Per_V2B_Interference
            else:
                print('Finish Running Real-FB D-Decision Test only!')
    ...
    def evaluate_training_diff_trials(self, num_episodes, num_test_step, opt_flag,
        ...
        if Train_Evaluation_Flag:
            ...
            Num_BS_Output = self.num_BS_Output
            ...
            for Episode_loop in range(self.num_Episodes):
                ...
                if Train_Evaluation_Flag:
                    ...
                    curr_sim_set = 'Train-Result' + '-RealFB-' + str(Num_D2D_feedback) + '-Batch-' + str(BATCH_SIZE) \
                                   + '-Gamma-' + str(GAMMA) + '-BSOutput-' + str(Num_BS_Output) \
                                   + '-V2Iweight-' + str(V2I_Weight)
        ...
        if RA_Flag and Opt_Flag:
            print('Finish evaluating the Real-FB D-Decision with Optimal Scheme!')
            ...
            if RA_Flag:
                print('Finish evaluating the Real-FB D-Decision without Optimal Scheme!')
                return Evaluated_Opt_Expect_Return, \
                       Evaluation_Return_per_Episode, Evaluation_Reward_per_Episode, \
                       RA_Expect_Return, RA_Reward
            else:
                print('Finish evaluating the Real-FB D-Decision only!')
                return Evaluation_Return_per_Episode, Evaluation_Reward_per_Episode
