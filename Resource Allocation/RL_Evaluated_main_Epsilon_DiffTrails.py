# implement the Evaluation main function for the D-Decision scheme with Real Feedback
...
def run_test(curr_RL_Config, BS_Agent, test_seed_sequence, Fixed_Epsilon, num_evaluate_trials):
    ...
    if Opt_Flag:
        ...
        # plot mean return and standard error in training for D-Decision and Random Action schemes
        ...
        label_str = 'D-Decision with ' + '$\epsilon = $' + str(Fixed_Epsilon)
        x = my_x_ticks
        ...
        # plot mean return and standard error in training for D-Decision
        Mean_Return_per_Episode = np.mean(Evaluation_Return_per_Episode, axis=0)
        Std_Return = np.std(Evaluation_Return_per_Episode, axis=0)
        Shape_Return = Evaluation_Return_per_Episode.shape
        # standard error
        ...
        # plot mean return in training for D-Decision
        y = Mean_Return_per_Episode
        plt.figure()
        x = my_x_ticks
