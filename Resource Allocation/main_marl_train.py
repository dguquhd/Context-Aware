...
def get_state(env, idx=(0,0), ind_episode=1., epsi=0.02):
    ...
    # print('>>>>>>>V2V_fast>>>>>>>>>\n', V2V_fast)
    # print('>>>>>>>>v2v_fastfading>>>>>>>>\n', env.V2V_channels_with_fastfading)
    # print('>>>>>>>>>>>V2V_abs>>>>>>>>\n', env.V2V_channels_abs[:, env.vehicles[idx[0]].destinations[idx[1]]])
...
with g.as_default():
    ...
    w_1 = tf.Variable(tf.truncated_normal([n_input, n_hidden_1], stddev=0.1), tf.float16)
    w_2 = tf.Variable(tf.truncated_normal([n_hidden_1, n_hidden_2], stddev=0.1), tf.float16)
    w_3 = tf.Variable(tf.truncated_normal([n_hidden_2, n_hidden_3], stddev=0.1), tf.float16)
    w_4 = tf.Variable(tf.truncated_normal([n_hidden_3, n_output], stddev=0.1), tf.float16)

    b_1 = tf.Variable(tf.truncated_normal([n_hidden_1], stddev=0.1), tf.float16)
    b_2 = tf.Variable(tf.truncated_normal([n_hidden_2], stddev=0.1), tf.float16)
    b_3 = tf.Variable(tf.truncated_normal([n_hidden_3], stddev=0.1), tf.float16)
    b_4 = tf.Variable(tf.truncated_normal([n_output], stddev=0.1), tf.float16)
    ...
    # g_loss = tf.cast(g_loss1, dtype = tf.float16)
    # tf.to_float16(
    #     g_loss,
    #     name='ToFloat'
    # )
    ...
    w_1_p = tf.Variable(tf.truncated_normal([n_input, n_hidden_1], stddev=0.1), tf.float16)
    w_2_p = tf.Variable(tf.truncated_normal([n_hidden_1, n_hidden_2], stddev=0.1), tf.float16)
    w_3_p = tf.Variable(tf.truncated_normal([n_hidden_2, n_hidden_3], stddev=0.1), tf.float16)
    w_4_p = tf.Variable(tf.truncated_normal([n_hidden_3, n_output], stddev=0.1), tf.float16)

    b_1_p = tf.Variable(tf.truncated_normal([n_hidden_1], stddev=0.1), tf.float16)
    b_2_p = tf.Variable(tf.truncated_normal([n_hidden_2], stddev=0.1), tf.float16)
    b_3_p = tf.Variable(tf.truncated_normal([n_hidden_3], stddev=0.1), tf.float16)
    b_4_p = tf.Variable(tf.truncated_normal([n_output], stddev=0.1), tf.float16)
...
def q_learning_mini_batch(current_agent, current_sess):
    ...
    a = [optim, g_loss]
    _, loss_val = current_sess.run(a, {g_target_q_t: batch_target_q_t, g_action: batch_action, x: batch_s_t})
    return loss_val
            ...
            for i in range(n_veh):
                for j in range(n_neighbor):
                    ...
                    if time_step % mini_batch_step == mini_batch_step-1:
                        ...
                        if i == 0 and j == 0:
                            print('step:', time_step, 'loss:', loss_val_batch, 'reward:', train_reward)
...
if IS_TEST:
    ...
    for idx_episode in range(n_episode_test):
        ...
        for test_step in range(n_step_per_episode):
            ...
            V2I_rate, V2V_success, V2V_rate, V2V_success1 = env.act_for_testing(action_temp)
            ...
            # V2V_demand_per_episode.append(np.average(demand_marl[idx_episode, test_step+1,2,:]))
            ...
            if test_step == n_step_per_episode - 1:
                ...
                V2V_success_list1.append(V2V_success1)
        ...
        print('marl:', round(np.average(V2I_rate_per_episode), 2), 'rand:', round(np.average(V2I_rate_per_episode_rand), 2))
        print('marl:', V2V_success_list[idx_episode], V2V_success_list1[idx_episode], 'rand:', V2V_success_list_rand[idx_episode])

    print('-------- marl -------------')
    print('n_veh:', n_veh, ', n_neighbor:', n_neighbor, ', n_RB:', n_RB)
    print('Sum V2I rate:', round(np.average(V2I_rate_list), 2), 'Mbps')
    print('Pr(V2V success):', round(np.average(V2V_success_list), 4))
    print('Pr(V2V success1):', round(np.average(V2V_success_list1), 4))
    ...
    # x = range(n_episode_test)
    # y_1 = V2V_demand_per_episode
    # y_1_a = [t / 8 for t in y_1]
    # # y_2 = V2V_success_list_rand
    # # for i in range(n_episode):
    # #     step.append(i)
    # # print(y)
    # fig, ax = plt.subplots()
    # # ax.scatter(x, y, s=1)
    # plt.plot(x, y_1_a, color='#4169E1', alpha=0.8, linewidth=1)
    # # plt.plot(x, y_2, color='#FF0000', alpha=0.8, linewidth=1)
    # ax.set_title("", fontsize=24)
    # ax.set_xlabel("Time step", fontsize=14)
    # ax.set_ylabel("Remaining V2V payload", fontsize=14)
    # plt.show()
    ...
    # power_marl_path = os.path.join(current_dir, "model/" + label + '/power_marl.mat')
    # scipy.io.savemat(power_marl_path, {'power_rand': power_marl})
