...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, n_neighbor):
        ...
        self.demand_size = np.inf
        # self.demand_size = 20

        self.V2V_Interference_all = np.zeros((self.n_Veh, self.n_neighbor, self.n_RB)) + self.sig2
        self.V2V_Interference_all1 = np.zeros((self.n_Veh, self.n_neighbor, self.n_RB)) + self.sig2
    ...
    def add_new_vehicles_by_number(self, n):
        ...
        for i in range(n):
            ...
            self.add_new_vehicles(start_position, start_direction, random.gauss(V, sigma))
    ...
    def renew_channel(self):
        ...
        self.V2V_channels_abs1 = np.zeros((len(self.vehicles), len(self.vehicles)))
        ...
        self.V2V_channels_abs1 = self.V2V_pathloss
        # print('>>>>>>>abs>>>>>>>>>\n', self.V2V_channels_abs)
    ...
    def renew_channels_fastfading(self):
        ...
        V2V_channels_with_fastfading = np.repeat(self.V2V_channels_abs[:, :, np.newaxis], self.n_Veh, axis=2)
        V2V_channels_with_fastfading1 = np.repeat(self.V2V_channels_abs1[:, :, np.newaxis], self.n_Veh, axis=2)
        ...
        # print('>>>>>>>V2V_channels_fastfading>>>>>>>\n', self.V2V_channels_with_fastfading.shape)
        # print(1/0)
        ...
        return V2V_channels_with_fastfading
    ...
    def Compute_Performance_Reward_Train(self, actions_power):
        ...
        V2V_Signal1 = np.zeros((len(self.vehicles), self.n_neighbor))
        V2V_channels_with_fastfading = self.renew_channels_fastfading()
        ...
        for i in range(self.n_RB):  # scanning all bands
            ...
            for j in range(len(indexes)):
                ...
                V2V_Signal1[indexes[j, 0], indexes[j, 1]] = 10 ** ((self.V2V_power_dB_List[power_selection[indexes[j, 0], indexes[j, 1]]]- V2V_channels_with_fastfading[indexes[j][0], receiver_j, i] + self.V2V_channels_with_fastfading[indexes[j][0], receiver_j, i] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)
        ...
        return V2I_Rate, V2V_Rate, reward_elements, V2V_Signal1
    ...
    def Compute_Performance_Reward_Test_rand(self, actions_power):
        ...
        return V2I_Rate, V2V_Rate, V2V_Signal1

    def Compute_Interference(self, actions):
        V2V_Interference = np.zeros((len(self.vehicles), self.n_neighbor, self.n_RB)) + self.sig2
        V2V_Interference1 = np.zeros((len(self.vehicles), self.n_neighbor, self.n_RB)) + self.sig2
        ...
        for i in range(self.n_RB):
            for k in range(len(self.vehicles)):
                for m in range(len(channel_selection[k, :])):
                    ...
                    V2V_Interference1[k, m, i] += 10 ** ((self.V2I_power_dB - V2V_channels_with_fastfading[i][self.vehicles[k].destinations[m]][i] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)
            ...
            for j in range(len(channel_selection[i, :])):
                for k in range(len(self.vehicles)):
                    for m in range(len(channel_selection[k, :])):
                        ...
                        V2V_Interference1[k, m, channel_selection[i, j]] += 10 ** ((self.V2V_power_dB_List[power_selection[i, j]]
                                                                                   - V2V_channels_with_fastfading[i][self.vehicles[k].destinations[m]][channel_selection[i,j]] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)
        self.V2V_Interference_all = 10 * np.log10(V2V_Interference)
        self.V2V_Interference_all1 = V2V_Interference1


    def act_for_training(self, actions):

        action_temp = actions.copy()
        V2I_Rate, V2V_Rate, reward_elements, V2V_Signal1 = self.Compute_Performance_Reward_Train(action_temp)
    ...
    def act_for_testing(self, actions):
        ...
        V2V_success1 = np.average(np.exp((-r0 * self.V2V_Interference_all1)/V2V_Signal1))
        # print(np.average(V2V_signal1))
        # print(V2V_channels_with_fastfading.shape)

        return V2I_Rate, V2V_success, V2V_Rate, V2V_success1

    def act_for_testing_rand(self, actions):
        r0 = 10
        action_temp = actions.copy()
        V2I_Rate, V2V_Rate, V2V_Signal1 = self.Compute_Performance_Reward_Test_rand(action_temp)
        # V2V_success = 1 - np.sum(self.active_links_rand) / (self.n_Veh * self.n_neighbor)  # V2V success rates
        V2V_success = np.average(np.exp((-r0 * self.V2V_Interference_all1)/V2V_Signal1))
        # V2V_success = np.exp((-r0 * np.sum(self.V2V_Interference_random/10)) / np.sum(V2V_signal))
